 ...
 * - Get list of ingredients that are related, but
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
	...
	for(int i = 0; i < MAT_SIZE; i++)
	{
		...
		{ 
			in = 0;
			strcpy(ingredient, ingredients[i]);
	...
	if(dist < k)
	{
		while(temp != NULL)
		{
			h = related_k_dist(h, ingredients[temp->x], k, dist + 1);
			temp = temp->next;
		}
	}
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
	...
	h = related_k_dist(h, source_ingredient, k_source, 0);
	avoid_head = related_k_dist(avoid_head, avoid, k_avoid, 0);

	intNode *traverseh = h;
	intNode *traversea = avoid_head;
	
	int in = 0;
	while(traverseh != NULL)
	{
		in = 0;
		while(traversea != NULL)
		{
			if(traverseh->x == traversea->x)
			{
				in = 1;
				break;
			}
		...
		if(in != 1)
			new_head = insertInt(new_head, traverseh->x);
		
		traversea = avoid_head;
		traverseh = traverseh->next;
	}
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    ******/
	...
	char compatible_ingredient[MAX_STR_LEN];
	int max_value = 0;
	
	for(int i = 0; i < 10; i++)
	{
		if(strcmp(recipe[i], "") != 0 && strcmp(recipe[i], to_change) != 0)
		{
			...
			for(int j = 0; j < MAT_SIZE; j++)
			{
				if(AdjMat[index][j] > 0)
				{
					if(AdjMat[index][j] > max_value)
					{
						max_value = AdjMat[index][j];
						strcpy(compatible_ingredient, ingredients[j]);
					}
				}
			}
		}
	...
	for(int i = 0; i < 10; i++)
	{
		...
		{
			strcpy(recipe[i], compatible_ingredient);
			break;
		}
	}
