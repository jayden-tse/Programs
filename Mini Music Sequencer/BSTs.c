...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     */
	 ...
	 BST_Node *new_music = NULL;
	 
	 new_music = (BST_Node *)calloc(1, sizeof(BST_Node));

	 if(new_music != NULL)
	 {
		 if(bar >= 0 && index >= 0.0 && index <= 1.0)
		 {
			 new_music->key = (10.0*bar) + index;
			 new_music->freq = freq;
			 new_music->bar = bar;
			 new_music->index = index;
			 new_music->left = NULL;
			 new_music->right = NULL;
		 }
		 else if(bar < 0)
			 printf("Invalid bar (bar must be greater than or equal to 0)\n");
		 else
			 printf("Invalid index (index ranges between 0.0 to 1.0)\n");
	 }
	 else
...
BST_Node *find_successor(BST_Node *right_child_node)
{
	...
	BST_Node *traverse = right_child_node;
...
void helper(BST_Node *root, double wantfreq, double changefreq)
{
	if(root->freq == wantfreq)
		root->freq = changefreq;
	if(root->left != NULL)
		helper(root->left, wantfreq, changefreq);
	if(root->right != NULL)
		helper(root->right, wantfreq, changefreq);
}
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
	 ...
	 if(root != NULL)
	 {
		 ...
		 if(wfound == 1 && cfound == 1)
			 helper(root, wantfreq, changefreq);

	 }

}

BST_Node *search(BST_Node *root, BST_Node *new_tree, int semitones, double time_shift)
{
	int found = 0;
	int i = 0;
	while(found == 0)
	...
	if(found == 1 && (i + semitones >= 0 && i + semitones <= 99) && (root->index + time_shift >= 0.0 && root->index + time_shift < 1.0))
	{
		BST_Node *new_music = NULL;
		new_music = newBST_Node(note_freq[i + semitones], root->bar, root->index + time_shift);
		new_tree = BST_insert(new_tree, new_music);

	}
	if(root->left != NULL)
		new_tree = search(root->left, new_tree, semitones, time_shift);
	if(root->right != NULL)
		new_tree = search(root->right, new_tree, semitones, time_shift);
	
	return new_tree;

}

void insert(BST_Node *root, BST_Node *new_tree)
{
	
	if(new_tree->left != NULL) 
		insert(root, new_tree->left);

	if(new_tree->right != NULL)
		insert(root, new_tree->right);
	
	BST_Node *new_node = NULL;
	new_node = newBST_Node(new_tree->freq, new_tree->bar, new_tree->index);
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     * positive or negative. A typicl value for semitones
	...
	if(root != NULL)
	{
		BST_Node *new_tree = NULL;
		
		new_tree = search(root, new_tree, semitones, time_shift);

		if(new_tree != NULL)
		{
			insert(root, new_tree);
			delete_BST(new_tree);
		}
